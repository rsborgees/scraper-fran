...
async function send10DressToWebhook() {
    console.log('🚀 Starting: Sending 10 Dress To items to webhook...');
    ...
    try {
        ...
        // Filter and sort by priority (newest first or unsent)
        ...
        const targetItems = dresstoItems.slice(0, 50); // Get a pool for scraping
        console.log(`🔍 Scrapping up to 10 products from a pool of ${targetItems.length} items...`);

        // 3. Scrape items
        const results = await scrapeSpecificIdsGeneric(context, targetItems, 'dressto', 10, { maxAgeHours: 0.01 });
        ...
        if (results.products && results.products.length > 0) {
            ...
            if (webhookResult.success) {
                ...
                console.log('✅ History and Stats updated.');
            } else {
