...
async function testFarmNovidadesHybrid() {
    console.log('🚀 INICIANDO TESTE: FARM NOVIDADES (HYBRID NODE + BROWSER)');

    const { browser, page } = await initBrowser();

    try {
        console.log('🌐 Navegando para a home para estabelecer sessão...');
        await page.goto('https://www.farmrio.com.br', { waitUntil: 'domcontentloaded' });
        await page.waitForTimeout(3000);

        // Get cookies to use with axios to avoid 400/403
        const cookies = await page.context().cookies();
        const cookieHeader = cookies.map(c => `${c.name}=${c.value}`).join('; ');

        console.log(`\n🔍 Buscando via API (Node + Cookies): ${API_SEARCH_NOVIDADES}`);

        const response = await axios.get(API_SEARCH_NOVIDADES, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36',
                'Cookie': cookieHeader,
                'Referer': 'https://www.farmrio.com.br',
        ...
        console.log(`\n✅ API retornou ${productsJson.length} produtos. Buscando os primeiros 2 em estoque...`);
        ...
        for (const pData of productsJson) {
            ...
            if (!url || typeof url !== 'string') {
                console.log('⚠️ URL missing in data.');
                continue;
            }
